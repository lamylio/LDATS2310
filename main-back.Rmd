...
gbm.best.oob = gbm.perf(model.gbm.2, method = "OOB")
gbm.best.cv = gbm.perf(model.gbm.2, method = "cv")
gbm.best.test = gbm.perf(model.gbm.2, method = "test")

par(mfrow=c(2,2))
plot(model.gbm.2, i.var = 1)
plot(model.gbm.2, i.var = 2)
plot(model.gbm.2, i.var = 3)
plot(model.gbm.2, i.var = 4)

install.packages('dismo')
library(dismo)
brtTuning<- gbm.step(data=base_train,
   gbm.x = 1:8,
   gbm.y = 10,
   family = "poisson",
   tree.complexity = 4,
...
model.neural.1 = neuralnet(formule.neural, data=neural_train, hidden=5, threshold=1, lifesign = "full",lifesign.step = 100,err.fct="poisson")
...
neural.calib.1 = neuralnet(formule.neural, data=neural_calib, hidden=1, threshold=1, lifesign = "full",lifesign.step = 100,err.fct="poisson", rep = 5)
neural.calib.2 = neuralnet(formule.neural, data=neural_calib, hidden=2, threshold=1, lifesign = "full",lifesign.step = 100,err.fct="poisson", rep = 5)
neural.calib.3 = neuralnet(formule.neural, data=neural_calib, hidden=3, threshold=1, lifesign = "full",lifesign.step = 100,err.fct="poisson", rep = 5)
...
neural.valid.1 = compute(neural.calib.1, covariates.neural(neural_valid), 50, neural_valid$Frequency, neural_valid$Exposure) 

neural.valid.2 = compute(neural.calib.2, covariates.neural(neural_valid), 50, neural_valid$Frequency, neural_valid$Exposure) 

neural.valid.3 = compute(neural.calib.3, covariates.neural(neural_valid), 50, neural_valid$Frequency, neural_valid$Exposure)
...
ggplot() + aes(x=3:6)+ geom_line(aes(y=model.neural.valid, col="red"))
